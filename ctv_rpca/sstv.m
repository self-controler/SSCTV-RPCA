function [Xten,Sten] = sstv(Yten,lambda)
sizeD =size(Yten);
mu = 0.01;
rho = 1.25;
maxIter = 200;
...
while iter<maxIter
    ...
    numer1   = reshape( diffT_p + mu*(Yten(:)-Sten(:)) + M1(:), sizeD);
    x        = real( ifftn( fftn(numer1) ./ (mu*Eny_z + mu ) ) );
    ...
    G3       = reshape(x,[h,w,d]);
    %% update G31 and G32
    G31      = softthre(reshape(diff_x(G3,sizeD),sizeD)+M3/mu,1/mu); 
